...
class Rossmann(object):
    ...
    def feature_engineering(self, df2):
        ...
        #filtragem das lojas nao abertas e que nao venderam
        df2  = df2[df2['open'] != 0]
    ...
    def data_preparation(self, df5):
        ...
        #ciclico
        #month(aplico sobre a 12 meses de rotação)
        df5['month_sin'] = df5['month'].apply(lambda x: np.sin(x*(2*np.pi/12)))
        df5['month_cos'] = df5['month'].apply(lambda x: np.cos(x*(2*np.pi/12)))

        #day(aplico sobre a 30 dias de rotação)
        df5['day_sin'] = df5['day'].apply(lambda x: np.sin(x*(2*np.pi/30)))
        df5['day_cos'] = df5['day'].apply(lambda x: np.cos(x*(2*np.pi/30)))

        #day_of_week (aplico sobre 7 dias na semana de rotação)
        df5['day_of_week_sin'] = df5['day_of_week'].apply(lambda x: np.sin(x*(2*np.pi/7)))
        df5['day_of_week_cos'] = df5['day_of_week'].apply(lambda x: np.cos(x*(2*np.pi/7)))

        #week_of_year (aplico sobre a 52 semanas num ano de rotação)
